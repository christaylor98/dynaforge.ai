...
This guide introduces the first three maturity tiers for Codexa.ai, derived from Change Requests `CR001 → CR001.3` and the consolidated requirements in `REQUIREMENTS_1_1.md`. Each level describes the minimum processes, artifacts, and agent involvement expected before advancing to the next tier.
