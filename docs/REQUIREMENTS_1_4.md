# CR004 1.4 Combined Requirement Updates

## Context
- Change sequence `CR001 → CR001.1 → CR001.2 → CR001.3 → CR002 → CR003 → CR004` establishes a change-centric governance model, discovery-first system understanding, and now formalizes Codexa’s hybrid operating model.
- This document consolidates `docs/REQUIREMENTS_1_1.md`, `docs/change_ideas_002.md`, the CR003 discovery initiative, and CR004 operating-model changes so requirement owners have a single source of truth for all functional requirements introduced to date.
...
| --- | --- | --- | --- |
| FR-01 Project Manager | - Orchestrate the RA→IA→IM→QA→TQA→GO loop and coordinate Change Evaluator outcomes.<br>- Detect `maturity_level` from project metadata and activate only the agents required for that level.<br>- Maintain change metrics, maturity state, and ensure Governance Officer reports are consumed before approvals.<br>- Run discovery pre-flight to confirm `.codexa/manifests/system_manifest.yaml` and `.codexa/manifests/change_zones.md` exist, log discovery mode (`quick`, `deep`, `code-only`), and trigger System Model Graph refreshes before decomposing workstreams.<br>- Resolve the configuration root by locating `.codexa/config.yaml` (falling back to global defaults) and persist `{config_root, extends_from}` in run metadata before dispatching agents. | `agents/project_manager.py`, `.codexa/config.yaml`, change dashboards, discovery CLI. | CR001–CR001.3, CR003, CR004 |
| FR-02 Status Documentation | - Embed per-change and maturity summaries in `PROJECT_OVERVIEW.md` / `PROJECT_DETAIL.md`.<br>- Cross-link to `CHANGELOG.md`, `IM_PROGRESS.md`, `GOVERNANCE_REPORT.md`, and maturity upgrade records.<br>- Surface discovery artifacts (latest manifest hash, change zone summaries, understanding coverage) and highlight the “Software re-imagined — from understanding to evolution” ethos in hero docs.<br>- Document the resolved configuration lineage (project vs global) and link to `.codexa/README.md` plus template version hashes. | PM agent, Governance Officer, Documentation owners. | CR001.1–CR001.3, CR003, CR004 |
...
| FR-32 | Maintain `.codexa/project_metadata.yaml` (mirrored to `PROJECT_METADATA.md` for human readability) capturing maturity level, last review, criteria, next target level, and any `extends:` pointers to global defaults; log provenance for each update. | PM / Governance Officer | Single source for maturity-aware agents. |
...
| FR-38 | Deliver discovery pipelines (agents + conversational prompts + CLI) that generate `.codexa/manifests/system_manifest.yaml`, `.codexa/manifests/change_zones.md`, and `.codexa/manifests/intent_map.md` across quick, deep, and code-only modes, streaming progress telemetry and writing iteration history to `docs/status/iteration_log.md`. | Analyzer / Platform | Humans configure runs via `.codexa/config.yaml`; manifests are repo-tracked canonical outputs aligned with CR003–CR004. |
| FR-39 | Maintain a System Model Graph expressed as versioned YAML projections under `.codexa/manifests/` linking code structure, intent, requirements, tests, and risk nodes, enriched with iteration metadata and manifest hashes so conversational follow-ups remain replayable. | Platform Architecture | YAML is the source of truth; database caches may exist locally but are not first-class artifacts. |
...
| FR-42 | Scaffold and lint the `.codexa/` project root (`config.yaml`, `agents/`, `rules/`, `workflows/`, `manifests/`, `README.md`) ensuring required files exist before execution and migrations document compatibility with Spec-Kit layouts. | Platform Tooling / PM Agent | Block runs when structure is missing; include conversion scripts for legacy repos. |
| FR-43 | Provide a global control-plane bundle (`~/.config/codexa/`) with versioned defaults, reusable templates, and governance policies consumable via `extends:` to minimize duplication across projects. | Platform Architecture | Publish change logs and semantic versions; agents must log inherited template versions. |
| FR-44 | Implement CLI configuration discovery and validation (`codexa doctor config`) covering project-only, global-only, and hybrid scenarios, with automated tests verifying precedence and schema compliance. | CLI / Developer Experience | Surface resolved configuration lineage and highlight missing scaffolding or schema violations. |
...
- Discovery manifests (`.codexa/manifests/system_manifest.yaml`, `.codexa/manifests/change_zones.md`, `.codexa/manifests/intent_map.md`) are required artifacts under FR-38 and must be regenerated when discovery pre-flights detect drift.
- System Model Graph YAML projections under `.codexa/manifests/` serve as the canonical knowledge base for FR-39; agents may cache derived structures locally but must not commit databases.
...
- `.codexa/` directory scaffolding (FR-42) must remain lint-clean; CI guards should block merges if required files or schema versions are missing.
- Global defaults (`~/.config/codexa/`) and per-project overrides must record provenance hashes so audit/traceability artifacts can recreate configuration lineage (FR-43–FR-44).
...
## Next Actions
1. Update `docs/REQUIREMENTS.md` and related traceability assets to reflect all updates in this document, including FR-38–FR-44.
2. Finalize discovery manifest schemas and wire `codexa discover`/`summarize`/`suggest-change-zones` prototypes to produce repo-tracked outputs under `.codexa/manifests/`.
3. Inject discovery and configuration metadata into the normalized audit schema across Implementer, `/df.*` tooling, and Governance Officer pipelines.
...
6. Scaffold `.codexa/` linting plus Spec-Kit migration helpers; publish the global control-plane (`~/.config/codexa/`) bundle with version history.
7. Deliver `codexa doctor config` validation flow and associated automated tests enforcing hybrid discovery precedence.
8. Implement `_c_*` ignore patterns and optional `codexa clip` helper to operationalize the clipping convention.
